        ...
        <>
            <div className="m-w-[895px] w-full">
                <div className={"flex-col w-full mb-10 p-5 border border-[#161616] bg-[#FFFCF9] " + cabin.className}>
                    <div className=" text-neutral-900 text-[28px] font-bold  leading-[33.60px] mb-[12px]">{data?.news?.content?.title}</div>
                    <div className="flex gap-1 mb-[24px]">
                        ...
                        <div className="justify-start items-center gap-2 flex ml-2">
                            <div className="opacity-80 text-neutral-900 text-sm font-normal  leading-[16.80px]">{data.news.content.user.username}</div>
                            <div className="w-[5px] h-[5px] opacity-80 bg-neutral-900" />
                            <div className="text-neutral-900 text-opacity-80 text-sm font-normal  leading-[16.80px]">{formatDate(data.news.content.publishdate)}</div>
                        </div>
                    </div>

                    <Image alt="banner" height={487.44} width={857.55} className="mb-[24px] w-[855.58px] " src={data.news.content.image} />


                    <div dangerouslySetInnerHTML={{ __html: data?.news?.body }} className={"text-neutral-900 text-base font-normal  leading-normal mb-[20.28px] " + cabin.className} />
